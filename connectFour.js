...
// Declare constants rows and colums intitalized to 6 rows and 7 colums
const rows = 6;
const columns = 7

// Declare variable initialMatrix as a 2d array, 6 rows, 7 columns, initialized to all 0s
var initialMatrix = Array.from({ length: rows }, () => new Array(columns).fill(0));
...
//console.log("setPiece");
    ...
    // Current if/else should be wrapped inside a try{} block. // Switch players
    try{
        // Declare variable rows initialized to object document, method querySelectorAll, passing argument class ".grid-row"
        let rows = document.querySelectorAll(".grid-row");
...
function checkHorizontal()
{
    ...
    for(let row = 0; row < initialMatrix.length; row++){
        for(let col = 0; col < initialMatrix[0].length - 3; col++){
...
function checkNegativeDiagonal()
{
    // Write a nest for loop to iterate through the rows and columns
    for(let row = 0; row < initialMatrix.length - 3; row++){
        for(let col = 3; col < initialMatrix[0].length; col++){
